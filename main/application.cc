...
void Application::Initialize() {
    ...
    callbacks.on_playback_progress = [this](uint32_t playback_id,
...
void Application::HandleNetworkConnectedEvent() {
    ...
    if (state == kDeviceStateStarting ||
        ...
        if (activation_task_handle_ != nullptr) {
            ESP_LOGW(
                TAG,
                "Activation task already running");
            return;
        }
...
void Application::HandleNetworkDisconnectedEvent() {
    ...
    if (state == kDeviceStateConnecting ||
        ...
        ESP_LOGI(
            TAG,
            "Closing audio channel due to network disconnection");
...
void Application::ActivationTask() {
    ...
    ESP_LOGI(
        TAG,
        "Zello build detected - skipping OTA/activation checks");
...
void Application::CheckAssetsVersion() {
    ...
    if (!assets.partition_valid()) {
        ESP_LOGW(
            TAG,
            "Assets partition is disabled for board %s",
            BOARD_NAME);
    ...
    while (true) {
        ...
        if (!check) {
            ...
            if (retry_count >= MAX_RETRY) {
                ESP_LOGE(
                    TAG,
                    "Too many retries, exit version check");
...
void Application::InitializeProtocol() {
    ...
    ESP_LOGI(
        TAG,
        "Initializing Zello protocol");

    protocol_ =
        std::make_unique<ZelloProtocol>(
            ZELLO_USERNAME,
            ZELLO_PASSWORD,
            ZELLO_AUTH_TOKEN,
            ZELLO_CHANNEL);
    ...
    auto zello_ring_state =
        std::make_shared<
            ZelloIncomingRingState>();
#endif
                    ...
                    Schedule(
                        [this,
                         zello_ring_state]() {
                            ...
                            ESP_LOGI(
                                TAG,
                                "Incoming Zello call - playing Arabic announcement");
                            ...
                            ESP_LOGI(
                                TAG,
                                "Receiver announcement finished - playing buffered Zello audio");
...
void Application::StartZelloPtt() {
    Schedule([this]() {
        auto* zello =
            static_cast<ZelloProtocol*>(
                protocol_.get());

        if (zello == nullptr) {
            ESP_LOGE(
                TAG,
                "Zello protocol is not active");
...
ListeningMode
Application::GetDefaultListeningMode() const {
    return
        aec_mode_ == kAecOff
            ? kListeningModeAutoStop
            : kListeningModeRealtime;
...
void Application::ResetProtocol() {
    Schedule(
        [this]() {
            ...
            ESP_LOGW(
                TAG,
                "ResetProtocol() called - protocol_ was %s",
                protocol_
                    ? "valid"
                    : "already null");
